...
from knuto.secrets import _should_copy, kafka_secret_create, kafka_secret, _source_namespace_for_secret
...
class Test_should_copy(TestCase):
    ...
    def test_correct_namespace(self, globalconf):
        ...
        globalconf.kafka_user_topic_source_namespaces = set(['latest', 'ingestion-latest'])

        self.assertTrue(_should_copy("latest-something", "kafka", "latest", {"data":{"password":"pw"}}, logger))

        self.assertTrue(_should_copy("ingestion-latest-something", "kafka", "ingestion-latest", {"data":{"password":"pw"}}, logger))

        self.assertFalse(_should_copy("ingestion-latest-something", "kafka", None, {"data":{"password":"pw"}}, logger))

        self.assertFalse(
            _should_copy("wrong-something", "kafka", "wrong", {"data": {"password": "pw"}},
                         logger))
    ...
    def test_scram_secret(self, globalconf):
        ...
        self.assertFalse(
            _should_copy("ingestion-latest-something", "kafka", "ingestion-latest", {"data": {}},
                         logger))
...
class Test_kafka_secret_create(TestCase):
    ...
    def test_create(self, _should_copy,
        ...
        secret_obj = {"metadata":{"namespace":"kafka",
                                  "name":"ns-with-dash-test",
                                  "labels":{"strimzi.io/kind":"KafkaUser"}},
                      "data":{"password":base64.b64encode("pass".encode("utf-8"))}}

        ret = kafka_secret_create(secret_obj,
                            "kafka",
                            "ns-with-dash-test",
                            logger)

        _source_namespace_for_secret.assert_called_with('kafka', 'ns-with-dash-test', logger)
        _should_copy.assert_called_with('ns-with-dash-test', 'kafka', 'ns-with-dash', secret_obj, logger)
        ...
        self.assertEqual(secret_created.annotations, {'knuto.niradynamics.se/source': 'kafka/ns-with-dash-test'})
...
class Test_kafka_secret(TestCase):
    ...
    @patch("knuto.secrets._should_copy")
    @patch('knuto.secrets._source_namespace_for_secret')
    def test_update(self, _source_namespace_for_secret, _should_copy, _update_or_create, globalconf):
        _should_copy.return_value = True
...
class Test_Source_Namespace_For_Secret(TestCase):
    ...
    def test_with_correct_annotation(self, _load_kafkauser):
        logger = MagicMock()
        _load_kafkauser.return_value = MagicMock()
        _load_kafkauser.return_value.annotations = {'knuto.niradynamics.se/source': 'ns-with-dash/test'}
